...
http.route({
	...
	handler: httpAction(async (ctx, req) => {
		...
		try {
			...
			console.log("Received event:", result.type, result.data);
			...
			switch (result.type) {
				case "user.created":
                    ...
                    case "session.created":
                        ...
                        console.log("setUserOnline called for:", `${process.env.CLERK_APP_DOMAIN}|${result.data.user_id}`);
                        break; 
                    case "session.ended":
                        await ctx.runMutation(internal.users.setUserOffline, {
                            tokenIdentifier: `${process.env.CLERK_APP_DOMAIN}|${result.data.user_id}`,
                        });
                        console.log("setUserOffline called for:", `${process.env.CLERK_APP_DOMAIN}|${result.data.user_id}`);
                        break; 
			}
